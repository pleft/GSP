...
Mario.LevelState.prototype.Enter = function() {
    var levelGenerator = new Mario.LevelGenerator(318, 15),
        i = 0,
        scrollSpeed = 0,
        w = 0,
        h = 0,
    ...
    this.Paused = false;
    this.Layer = new Mario.LevelRenderer(this.Level, 318, 238);
    ...
    for (i = 0; i < 2; i++) {
        scrollSpeed = 4 >> i;
        w = ((((this.Level.Width * 16) - 318) / scrollSpeed) | 0) + 318;
        h = ((((this.Level.Height * 16) - 238) / scrollSpeed) | 0) + 238;
        bgLevelGenerator = new Mario.BackgroundGenerator(w / 32 + 1, h / 32 + 1, i === 0, this.LevelType);
        this.BgLayer[i] = new Mario.BackgroundRenderer(bgLevelGenerator.CreateLevel(), 318, 238, scrollSpeed);
    }
...
Mario.LevelState.prototype.Update = function(delta) {
    ...
    if (this.Camera.X > this.Level.Width * 16 - 318) {
        this.Camera.X = this.Level.Width * 16 - 318;
    }
    ...
    for (i = 0; i < this.Sprites.Objects.length; i++) {
        ...
        if (sprite !== Mario.MarioCharacter) {
            ...
            if (xd < -64 || xd > 318 + 64 || yd < -64 || yd > 238 + 64) {
...
Mario.LevelState.prototype.Draw = function(context) {
    ...
    if (this.Camera.Y > this.Level.Height * 16 - 238) {
        this.Camera.Y = this.Level.Height * 16 - 238;
    }
    ...
    if (Mario.MarioCharacter.WinTime > 0) {
        ...
        this.RenderBlackout(context, ((Mario.MarioCharacter.XDeathPos - this.Camera.X) | 0), ((Mario.MarioCharacter.YDeathPos - this.Camera.Y) | 0), (318 - t) | 0);
    }
